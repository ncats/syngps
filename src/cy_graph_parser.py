...
def add_node (G, node_id, node_metadata):
    ...
    if node_id not in G.nodes:
        ...
        raise Exception(f'[ERROR] Node of duplicate ID: {node_id} present in network. Terminating...')
...
def process_nodes (G, nodes):
    ...
    for n in nodes:
        ...
        if node_type == 'reaction':
            ...
            # We add the SUID (here: uuid) of nodes as node id since cyjs uses the SUID of start and end nodes to define edges
...
def add_edge (G, start_node, end_node, edge_metadata):
    ...
    """
    #print (f'adding edge: ({start_node}, {end_node}), nr.nodes: {len(G.nodes)}')
...
def process_edges (G, edges):
    ...
    for e in edges:
        ...
        #edge_metadata['edge_label'] = e['data']['name']
