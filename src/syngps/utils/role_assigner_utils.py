...
def normalize_roles(rxsmiles: str, sort_ions: bool = False) -> str:
    ...
    for substance in all_substances:
        ...
        if "." in substance["smiles"]:
            ...
            if sort_ions:
                # Sort fragments by charge, cations first, anions last
                fragments = sorted(fragments, key=lambda s: (get_charge(s), s))
            substance["smiles"] = ".".join(fragments)
