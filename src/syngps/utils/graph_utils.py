...
from syngps.utils import graph_hash_utils
import logging
...
def get_yield_predicted(node_attrs: dict) -> Optional[float]:
    """
    Safely extract the predicted yield from a node, prioritizing 'yield_info.yield_predicted'.

    Args:
        node_attrs (dict): The attribute dictionary for a node.

    Returns:
        Optional[float]: The predicted yield, or None if not found.
    """
    yield_info = node_attrs.get("yield_info", {})
    if isinstance(yield_info, dict) and "yield_predicted" in yield_info:
        return yield_info["yield_predicted"]
    return node_attrs.get("yield_predicted")
...
def get_zero_yield_reactions(G: DiGraph) -> List:
    ...
    for node_id, attrs in G.nodes(data=True):
        if attrs.get("node_type", "").lower() == "reaction":
            yield_val = get_yield_predicted(attrs)
            if yield_val == 0:
                zero_yield_nodes.append(node_id)
    return zero_yield_nodes
...
def remove_incompatible_reactions(
    ...
    """
    # Identify incompatible reactions
    ...
    children_of_target = get_childrens_of_target_molecule(G)
    parents_of_starting_materials = get_parent_nodes_of_starting_materials(G)
    zero_yield_rxns = get_zero_yield_reactions(G)

    # Update set with all incompatible types
    incompatible_reactions.update(children_of_target)
    incompatible_reactions.update(parents_of_starting_materials)
    incompatible_reactions.update(zero_yield_rxns)
    ...
    if incompatible_reactions:
        ...
        logger.debug(f"  - Ambiguous reactions: {len(ambiguous_rxns)}")
        logger.debug(f"  - Multi-product reactions: {len(multiproduct_rxns)}")
        logger.debug(f"  - General incompatibilities: {len(get_incompatible_reactions(G, ambiguous_rxns, multiproduct_rxns))}")
        logger.debug(f"  - Children of target molecule: {len(children_of_target)}")
        logger.debug(f"  - Parents of starting materials: {len(parents_of_starting_materials)}")
        logger.debug(f"  - Zero-yield reactions: {len(zero_yield_rxns)}")
...
def find_viable_route(C: DiGraph, target_module_node_id: Union[str, int]) -> Tuple[DiGraph, str]:
    """
    ...
    Returns:
        Tuple[DiGraph, str]: A tuple containing:
            DiGraph: A directed graph representing a viable synthesis route.
            str: The status of the synthesis route.
    """
...
def grab_yield_predicted(G: DiGraph) -> Dict[str, float]:
    ...
    """
    rxid_to_yield_predicted = {}
    for node in G.nodes:
        if G.nodes[node]["node_type"].lower() == "reaction":
            rxid = G.nodes[node]["node_label"]
            yield_info = G.nodes[node].get("yield_info", {})

            # Check if 'yield_predicted' is present, set to 0.0 if not
            if yield_info is None or "yield_predicted" not in yield_info:
                rxid_to_yield_predicted[rxid] = 0.0
                continue
            
            yield_predicted = yield_info.get("yield_predicted")
            ...
            if yield_predicted == "False":
                ...
                rxid_to_yield_predicted[rxid] = round(float(yield_predicted), 2) if yield_predicted is not None else 0.0

    return rxid_to_yield_predicted
