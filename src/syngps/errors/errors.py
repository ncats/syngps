...
class MultipleReactionFoundError(Exception):
    """Exception raised when multiple reaction nodes are found when only one is expected."""

    def __init__(self, rxid: str, message: str = "Multiple reaction nodes with rxid found"):
        self.rxid = rxid
        self.message = f"{message}: {rxid}"
        super().__init__(self.message)
