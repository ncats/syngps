...
def synth_route_to_adjacency_matrix(G: nx.DiGraph) -> Tuple[nx.DiGraph, np.ndarray, np.ndarray, Dict[int, str], Dict[str, int]]:
    ...
    nodes = list(G.nodes())
    for n in nodes:
        node_yield = G.nodes[n].get("yield", 0.0)
        if node_yield <= 0.0:
            raise ValueError("Invalid yield associated with node: {}".format(n))
        G.nodes[n]["one_over_yield"] = 1 / node_yield
        logger.debug(f"Node: {n}, yield: {node_yield}, one_over_yield: {G.nodes[n]['one_over_yield']}")
